...
window.onload = () => {
  ...
  if (scene) {
    ...
    scene.addEventListener("arjs-marker-found", (event) => {
      console.log(
        "Marker found:",
        event.detail.marker.id,
        event.detail.marker.type
      );
    });
...
function checkAREntities() {
  ...
  if (marker) {
    ...
    console.error(
      "a-marker not found. Ensure it is correctly defined in your HTML."
    );
  }
  ...
  if (model) {
    ...
    if (animation) {
      ...
      console.warn(
        "Animation component not found on the model, but it should be there."
      );
    }
    ...
    console.error(
      "Animated model (#animated-model) not found. Ensure it is correctly defined within the a-marker."
    );
  }
...
AFRAME.registerComponent("manual-rotation", {
  ...
  init: function () {
    ...
    this.modelEntity.addEventListener("touchstart", this.onTouchStart, {
      passive: false,
    });
  },
  ...
  onTouchStart: function (event) {
    if (event.touches.length === 1) {
      ...
      this.sceneEl.addEventListener("touchmove", this.onTouchMove, {
        passive: false,
      });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    ...
    console.error(
      "One or more critical UI elements (sidebar, buttons, model, cart controls, notification) not found."
    );
    return;
  }

  console.log(
    "All UI elements for sidebar, cart, and notification functionality found."
  );

  // --- Initialize currentSelectedDish with the default loaded model/dish ---
  const defaultDishItem = sidebar.querySelector(
    'ul li[data-model-src="assets/Bruschetta.glb"]'
  );
  if (defaultDishItem) {
    currentSelectedDish = {
    ...
    console.warn(
      "Could not pre-select the default dish. Ensure the Bruschetta item exists in HTML with correct data attributes."
    );
  }
  ...
  animatedModel.addEventListener("model-loaded", () => {
    console.log(
      "New 3D Model successfully loaded:",
      animatedModel.getAttribute("gltf-model")
    );
  ...
  dishItems.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      console.log(
        `Dish selected: ${dishName}, Model: ${modelSrc}, Price: ${dishPrice}`
      );
  ...
  function updateCartDisplay() {
    ...
    cartStatusDiv.textContent = `Cart: ${totalItems} item(s) - $${totalPrice.toFixed(
      2
    )}`;
  ...
  addToCartBtn.addEventListener("click", () => {
    ...
    const existingItemIndex = cart.findIndex(
      (item) => item.id === currentSelectedDish.id
    );
